...
function App() {
  ...
  return <main>
    ...
    <div className="grid">
      <section className="card source"><div className="cardhead"><div><span className="label">SOURCE NOTICE</span><h2>{activeNotice.title}</h2></div><span className="tag">Synthetic</span></div><div className="notice-tabs">{notices.map((item, i) => <button className={i === noticeIndex ? 'tab active-tab' : 'tab'} onClick={() => { setNoticeIndex(i); setRestriction(item.restriction); }} key={item.title}>{item.title}</button>)}</div><pre>{activeNotice.text}</pre><p className="caption">The extracted facts remain beside their source. The system does not infer missing details.</p></section>
