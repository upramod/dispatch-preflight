...
function App() {
  ...
  async function extractPdf(file: File) {
    ...
    try { const bytes = new Uint8Array(await file.arrayBuffer()); setExtractionProgress(25); let binary = ''; bytes.forEach(byte => binary += String.fromCharCode(byte)); setPdfUrl(URL.createObjectURL(file)); setExtractionProgress(45); setExtractionStatus('Sending extracted text to Azure OpenAI…'); const r = await fetch('/api/extract-pdf', { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify({pdfBase64: btoa(binary)}) }); setExtractionProgress(85); const data = await r.json(); if (!r.ok) throw new Error(data.error); setExtractedText(data.extractedText); setRestriction({ ...data, approved: false }); setExtractionProgress(100); setExtractionStatus(`Azure OpenAI · confidence ${Math.round(data.confidence * 100)}% · PDF text extracted`); } catch (e) { setExtractionProgress(0); setExtractionStatus(e instanceof Error ? e.message : 'PDF extraction failed'); }
  }
  return <main>
    ...
    <div className="grid">
      <section className="card source"><div className="cardhead"><div><span className="label">SOURCE NOTICE</span><h2>{pdfUrl ? 'Uploaded city PDF' : activeNotice.title}</h2></div><span className="tag">{pdfUrl ? 'User-provided' : 'Synthetic'}</span></div><div className="notice-tabs">{notices.map((item, i) => <button className={i === noticeIndex ? 'tab active-tab' : 'tab'} onClick={() => { setNoticeIndex(i); setRestriction(item.restriction); setPdfUrl(null); setExtractedText(null); setExtractionProgress(0); setExtractionStatus('Synthetic demo facts'); }} key={item.title}>{item.title}</button>)}</div>{pdfUrl ? <div className="source-columns"><div><span className="label">PDF PREVIEW</span><iframe title="Uploaded city notice" src={pdfUrl}/></div><div><span className="label">EXTRACTED TEXT</span><pre>{extractedText || 'Extracting PDF text…'}</pre></div></div> : <pre>{activeNotice.text}</pre>}<div className="upload"><label htmlFor="pdf-upload">Upload a city PDF</label><input id="pdf-upload" type="file" accept="application/pdf" onChange={e => e.target.files?.[0] && extractPdf(e.target.files[0])}/></div><button onClick={async () => { setExtractionProgress(20); setExtractionStatus('Extracting with Azure OpenAI…'); try { const r = await fetch('/api/extract', { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify({notice: activeNotice.text}) }); setExtractionProgress(85); const data = await r.json(); if (!r.ok) throw new Error(data.error); setRestriction({ ...data, approved: false }); setExtractionProgress(100); setExtractionStatus(`Azure OpenAI · confidence ${Math.round(data.confidence * 100)}%`); } catch (e) { setExtractionProgress(0); setExtractionStatus(e instanceof Error ? e.message : 'Extraction failed'); } }}>Extract with Azure OpenAI</button>{extractionProgress > 0 && <div className="progress-wrap"><div className="progress-track"><div className="progress-bar" style={{width: `${extractionProgress}%`}}/></div><span>{extractionProgress}%</span></div>}<p className="caption">{extractionStatus}. Facts remain beside their source. The system does not infer missing details.</p></section>
