...
function App() {
  ...
  async function extractPdf(file: File) {
    ...
    try { const bytes = new Uint8Array(await file.arrayBuffer()); let binary = ''; bytes.forEach(byte => binary += String.fromCharCode(byte)); const r = await fetch('/api/extract-pdf', { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify({pdfBase64: btoa(binary)}) }); const data = await r.json(); if (!r.ok) throw new Error(data.error); setRestriction({ ...data, approved: false }); setExtractionStatus(`Azure OpenAI · confidence ${Math.round(data.confidence * 100)}% · PDF text extracted`); } catch (e) { setExtractionStatus(e instanceof Error ? e.message : 'PDF extraction failed'); }
  }
  return <main>
