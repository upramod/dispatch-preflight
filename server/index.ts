...
const extractionSchema = { type: 'object', additionalProperties: false, properties: {
  street: { type: ['string', 'null'] }, crossStreet: { type: ['string', 'null'] },
  startsAt: { type: ['string', 'null'] }, endsAt: { type: ['string', 'null'] },
  confidence: { type: 'number' }, sourceQuote: { type: 'string' }
}, required: ['street', 'crossStreet', 'startsAt', 'endsAt', 'confidence', 'sourceQuote'] };

function json(res: any, status: number, body: unknown) { res.writeHead(status, { 'content-type': 'application/json', 'access-control-allow-origin': '*' }); res.end(JSON.stringify(body)); }
...
const server = createServer(async (req, res) => {
  ...
  if (!response.ok) return json(res, 502, { error: `Azure OpenAI returned ${response.status}` });
  const completion: any = await response.json();
  return json(res, 200, JSON.parse(completion.choices[0].message.content));
});
